...
-   **Forms & Validation**: React Hook Form + Zod _(Dependencies installed & patterns ready)_
-   **API Client**: Axios
-   **Data Visualization**: Recharts
-   **Icons**: Lucide React

_(Note: While the prompt requested Vite as a bundler, Next.js explicitly uses its own bundler via Turbopack/Webpack. We followed the strict requirement to use Next.js, hence no separate Vite config was included)._
...
1. **Install Dependencies** (from the workspace root or the `web` folder):
    ...
    _Note: Because this was scaffolded within a Turborepo environment, `legacy-peer-deps` might be necessary to bypass strict React 19 version checks with older Radix primitives._
