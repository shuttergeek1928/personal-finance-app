...
function DashboardContent() {
  ...
  const income = useMemo(() => transactions.filter(t => t.type === "income").reduce((acc, curr) => acc + curr.amount, 0), [transactions])
  ...
  const chartData = useMemo(() => {
    ...
    return Object.keys(dataMap).map(date => ({ date, amount: dataMap[date] })).reverse();
  }, [transactions])
    ...
    return (
      <div className="flex-1 p-8 space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {[1,2,3,4].map(i => <div key={i} className="h-32 bg-zinc-200 dark:bg-zinc-800 rounded-xl animate-pulse" />)}
        </div>
      </div>
    )
  }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-zinc-500 mt-1">Across all {accounts.length} accounts</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4">
          <CardHeader>
            ...
            <CardDescription>Your expenses over the last 30 days.</CardDescription>
          </CardHeader>
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="amount" fill="currentColor" className="fill-indigo-600 dark:fill-indigo-400" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <Card className="col-span-3">
          <CardHeader>
            ...
            <CardDescription>You have {transactions.length} transactions total finding your recent activity.</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                {transactions.slice(0, 5).map((t) => (
                  <div key={t.id} className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-zinc-500 dark:text-zinc-400">{t.category}</p>
                    </div>
                    <div className={`font-medium ${t.type === 'expense' ? 'text-zinc-900 dark:text-zinc-100' : 'text-emerald-600 dark:text-emerald-400'}`}>
