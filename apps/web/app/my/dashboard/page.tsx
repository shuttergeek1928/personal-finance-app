  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg px-3 py-2 text-sm z-50">
      ...
      {payload.map((p: any, i: number) => (
        <p
          key={i}
          style={{ color: p.color || p.fill || p.payload?.fill }}
...
export default function MyDashboardPage() {
  ...
  const [timePeriod, setTimePeriod] = useState<
    "ALL_TIME" | "THIS_MONTH" | "LAST_3_MONTHS" | "THIS_YEAR"
  >("THIS_MONTH");
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (
          ...
          const data = Array.isArray(accRes.value.data)
            ? accRes.value.data
            : [accRes.value.data];
          setAccounts(data);
        }
  ...
  const healthScore = useMemo(() => {
    ...
    const savingsRate =
      globalTotalIncome > 0
        ...
        : 0;
    const savingsScore =
      savingsRate > 0.2 ? 40 : savingsRate > 0 ? (savingsRate / 0.2) * 40 : 0;
    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-8 pb-20">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
        ...
        <Card className="flex flex-col justify-center items-center py-6 shadow-md border-indigo-100 dark:border-indigo-900/40 relative overflow-hidden">
          ...
          <CardTitle className="text-sm font-medium text-muted-foreground mb-4 z-10 w-full text-center">
            Financial Health
          </CardTitle>
          <div className="h-[120px] w-[120px]">
            <ResponsiveContainer width="100%" height="100%">
              <RadialBarChart
                ...
                barSize={12}
                data={[
                  { name: "Score", value: healthScore, fill: healthScoreColor },
                ]}
                startAngle={225}
                endAngle={-45}
              >
            ...
            <CardContent>
              ...
              {timePeriod === "THIS_MONTH" && (
                <div className="flex items-center gap-1 mt-1 text-xs">
                  ...
                  >
                    {momIncome.change >= 0 ? "↑" : "↓"}{" "}
                    {Math.abs(momIncome.change).toFixed(1)}%
                  </span>
            ...
            <CardContent>
              ...
              {timePeriod === "THIS_MONTH" && (
                <div className="flex items-center gap-1 mt-1 text-xs">
                  ...
                  >
                    {momExpense.change <= 0 ? "↓" : "↑"}{" "}
                    {Math.abs(momExpense.change).toFixed(1)}%
                  </span>
                  <span className="text-muted-foreground">vs last month</span>
                </div>
              )}
        ...
        <Card className="lg:col-span-2 shadow-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-lg flex items-center gap-2">
              <Activity className="h-5 w-5 text-indigo-500" /> 30-Day Spending
              Intensity
            </CardTitle>
            <CardDescription>
              Daily expense heatmap reflecting transaction volume
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="bg-zinc-50 dark:bg-zinc-950 p-6 rounded-xl border flex flex-col items-center min-h-[140px]">
                <div className="flex gap-1 mx-auto w-full items-center h-10 mb-6 px-1">
                  {expenseHeatmap.map((day, i) => (
                    <div
                      key={i}
                      className="flex-1 flex flex-col items-center group relative h-full justify-center"
                    >
                      <div
              ...
              <div className="space-y-3">
                {upcomingPayments.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="flex items-center justify-between p-3 rounded-lg border bg-white dark:bg-zinc-950 shadow-sm hover:shadow-md transition-all"
                    >
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg flex items-center gap-2">
                <PieChartIcon className="h-5 w-5 text-amber-500" /> Category
                Drill-down
              </CardTitle>
              {selectedCategory && (
                <Button
            ...
            <CardDescription>
              Click on a slice to view recent transactions
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="flex items-center gap-2 mb-4 p-3 bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400 rounded-lg border border-rose-100 dark:border-rose-900/30">
                      ...
                      <div>
                        <p className="font-medium text-sm">
                          Recent {selectedCategory} Expenses
                        </p>
                      </div>
                    </div>
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                      {drillDownTransactions.length > 0 ? (
                        drillDownTransactions.map((t) => (
                          <div
                            key={t.id}
                            className="flex justify-between items-center p-3 rounded-lg border bg-white dark:bg-zinc-950 shadow-sm"
                          >
                            <div>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg flex items-center gap-2">
                <BarChart3 className="h-5 w-5 text-indigo-500" /> Account
                Balances
              </CardTitle>
              <Link
                href="/my/accounts"
              ...
              <div className="grid gap-3">
                {obligations.liabilities.slice(0, 3).map((l) => {
                  ...
                  return (
                    <div
                      key={l.id}
                      className="bg-zinc-50 border dark:bg-zinc-950/50 rounded-xl p-4 transition-all hover:bg-white dark:hover:bg-zinc-950 hover:shadow-md"
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="font-semibold text-sm">{l.name}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {LiabilityTypeLabels[l.type]}
                          </p>
                        </div>
