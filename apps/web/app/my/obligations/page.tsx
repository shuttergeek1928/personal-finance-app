...
export default function ObligationsPage() {
  ...
  const [dashboard, setDashboard] = useState<ObligationDashboardDto | null>(null);
    ...
    return (
      <div className="p-8 flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">Loading your obligations...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-gradient-to-br from-rose-500 to-rose-700 text-white border-0 shadow-lg">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-rose-100">Outstanding Balance</CardTitle>
            <TrendingDown className="h-5 w-5 text-rose-200" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{fmt(d?.totalOutstandingBalance ?? 0)}</div>
            <p className="text-xs text-rose-200 mt-1">
              {d?.totalActiveLiabilities ?? 0} active loan{(d?.totalActiveLiabilities ?? 0) !== 1 ? "s" : ""}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Subscriptions / mo</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {d?.totalActiveSubscriptions ?? 0} active subscription{(d?.totalActiveSubscriptions ?? 0) !== 1 ? "s" : ""}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-indigo-500 to-indigo-700 text-white border-0 shadow-lg">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-indigo-100">Total / Month</CardTitle>
            <BarChart3 className="h-5 w-5 text-indigo-200" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{fmt(d?.totalMonthlyObligations ?? 0)}</div>
        ...
        {liabilities.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground/70 mt-1">Add a loan or EMI to start tracking.</p>
              <Link
                href="/my/obligations/liabilities"
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {liabilities.slice(0, 6).map((l) => {
              ...
              return (
                <Card key={l.id} className="group hover:shadow-lg transition-all cursor-pointer hover:border-indigo-300 dark:hover:border-indigo-700" onClick={() => router.push("/my/obligations/liabilities")}>
        ...
        {subscriptions.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground/70 mt-1">Add a subscription to start tracking.</p>
              <Link
                href="/my/obligations/subscriptions"
                className="mt-4 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 transition-colors"
              >
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {subscriptions.slice(0, 6).map((s) => (
              <Card key={s.id} className="group hover:shadow-lg transition-all cursor-pointer hover:border-indigo-300 dark:hover:border-indigo-700" onClick={() => router.push("/my/obligations/subscriptions")}>
                <CardContent className="p-5 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-semibold text-base">{s.name}</p>
                      <p className="text-xs text-muted-foreground">{s.provider} · {SubscriptionTypeLabels[s.type]}</p>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                      ...
                      {s.autoRenew ? (
                        <span className="inline-flex items-center gap-1"><RefreshCw className="h-3 w-3" /> Auto</span>
                      ) : "Manual"}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{BillingCycleLabels[s.billingCycle]}</p>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <CalendarClock className="h-3.5 w-3.5" />
                    Next: {new Date(s.nextBillingDate).toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })}
