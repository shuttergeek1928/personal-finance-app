...
export default function MyTransactionsPage() {
  ...
  const [toSourceType, setToSourceType] = useState<"account" | "card">(
    "account"
  );
  ...
  const summary = useMemo(() => {
    const totalBank = accounts.reduce(
      (acc, curr) => acc + (curr.balance?.amount || 0),
      0
    );
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        currency:
          sourceType === "account"
            ? selectedAccount?.balance?.currency || "INR"
            : selectedCard?.totalLimit?.currency || "INR",
        description,
        category,
  ...
  const getSourceName = (tx: Transaction) => {
    ...
    if (!isEmptyGuid(tx.creditCardId)) {
      ...
      return {
        name: card?.cardName || `${card?.bankName} Card` || "Credit Card",
        type: "card" as const,
      };
    }
  ...
  const getTypeIcon = (type: TransactionType) => {
    switch (type) {
      ...
      case TransactionType.Transfer:
        return <ArrowRightLeft className="h-5 w-5 text-indigo-500" />;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your cash flow and credit spending
          </p>
        </div>
              ...
              <div className="grid gap-6">
                            ...
                            <SelectItem key={card.id} value={card.id}>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-muted-foreground ml-auto">
                                  (Used: ₹
                                  {card.outstandingAmount?.amount?.toLocaleString()}
                                  )
                                </span>
                              </div>
                            </SelectItem>
                ...
                {type === "transfer" && (
                  <div className="space-y-4 rounded-xl border border-indigo-100 bg-indigo-50/10 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <Label className="text-xs font-bold uppercase tracking-wider text-indigo-600/70">
                        Destination
                      </Label>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            toSourceType === "account"
                              ? "bg-white dark:bg-indigo-700 text-indigo-700 dark:text-indigo-100 shadow-sm"
                              : "text-indigo-600/60 dark:text-indigo-400/60 hover:text-indigo-600 dark:hover:text-indigo-300"
                          )}
                        >
                          Bank
                        </button>
                    ...
                    {toSourceType === "account" ? (
                      ...
                      <div className="space-y-2">
                        <Select
                          ...
                          onValueChange={setToCreditCardId}
                          required
                        >
                          <SelectTrigger className="bg-background border-zinc-200 dark:border-zinc-800">
        ...
        <Card className="overflow-hidden border-none shadow-md bg-gradient-to-br from-indigo-500 to-indigo-600 text-white">
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-zinc-100/80 text-xs font-semibold uppercase tracking-wider">
                  Total Bank Balance
                </p>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1 text-orange-600 ml-0.5">
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1">
                  ₹{summary.recentSpending.toLocaleString()}
                </h3>
              </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Select
                  ...
                  onValueChange={(val) => setFilterSourceType(val as any)}
                >
                        ...
                        >
                          ...
                          >
                            {tx.type === TransactionType.Income
                              ...
                              ? "-"
                              : ""}
                            {tx.money?.amount?.toLocaleString("en-IN", {
                              style: "currency",
                              currency: tx.money?.currency || "INR",
                            })}
                          </td>
                        </tr>
                      );
